...
import type { UserIntent, ExecutionPlan, PlanStep } from './types';
...
export class SyncClient {
      ...
      try {
        ...
        this.ws.onopen = () => {
          ...
          if (this.config.authToken) {
            this.send('auth', { token: this.config.authToken });
          }
  ...
  send(type: string, payload: unknown): void {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      console.warn('Not connected to Desktop');
      return;
    }
   ...
   */
  async requestObserve(): Promise<UserIntent[]> {
    this.send('context_request', {});
    // The response will come through intent_detected message
    return new Promise((resolve, reject) => {
  ...
  /**
   * Send page context to Desktop for analysis
   */
  ...
  private handleMessage(message: SyncMessage): void {
    ...
    switch (message.type) {
      ...
      case 'auth_success':
        console.log('Authenticated with Desktop');
        break;
