...
**Shadow** 是一款 **AI 驱动的智能任务感知与执行助手**，通过持续观察用户的工作环境（屏幕、窗口、剪贴板、文件系统），主动理解用户意图，生成可执行的任务建议，并在用户确认后自动执行。
...
| 特性 | Shadow | Claude Code/Cursor | Eigent | 传统自动化工具 |
|------|---------|-------------------|--------|---------------|
...
```
[触发] 用户复制了一段错误信息到剪贴板
[感知] Shadow 检测到剪贴板内容是错误日志
[分析] AI 分析错误原因，搜索相关解决方案
[建议] 显示 3 个可能的解决方案
[执行] 用户选择方案后，Shadow 自动执行修复命令
```
...
Shadow 采用 **MemOS** 作为核心记忆管理系统，实现多维度的本地数据存储和智能检索。
...
| **本地 LLM** | Ollama | Latest | 本地模型（推荐 Qwen3/Qwen2.5-VL）|
...
import { ShadowEngine, TaskSuggestion, ExecutionResult } from '@hawkeye/core';
...
import { ShadowPlugin, PluginContext } from '@hawkeye/core';

// 定义插件
const myPlugin: ShadowPlugin = {
...
| 特性 | Shadow | Eigent | Claude Code | Cursor | Raycast AI |
|------|---------|--------|-------------|--------|------------|
...
**Shadow**: 完全不需要 Prompt，系统根据上下文自动构建
...
**传统 AI**: 直接执行或给出答案
**Shadow**: Human-in-the-Loop，展示优缺点风险，用户确认后才执行，所有操作可回滚
...
**传统 AI**: 效果取决于用户技能
**Shadow**: 效果由系统保证，自动学习用户习惯，新手和专家得到同样优质体验
...
**传统 AI**: 订阅付费 / 高昂部署成本
**Shadow**: 完全免费开源，可用免费本地模型

### 16.3 Shadow vs 传统 AI 对比

| 痛点 | 传统 AI 方式 | Shadow 方式 |
|------|-------------|-------------|
| **交互模式** | 用户主动提问 | AI 主动建议 |
...
这个根本性的差异使得 Shadow 能够服务那些"不知道问什么"的 99.9% 的普通人。

### 16.5 给普通人的 AI

Shadow 的核心价值可以总结为 **"五个零"**：
...
async selectAIBackend(): Promise<AIBackend> {
  ...
  if (await this.detectOllama()) {
    return { type: 'ollama', model: 'qwen3:8b' };
  }
...
#### 17.8.4 Shadow 交互方案设计

基于以上研究，Shadow 采用 **"观察-生成-选择"** 交互模式：

**核心原则：用户全程不需要输入任何文字，所有交互通过点击完成。**

```
┌─────────────────────────────────────────────────────────────────┐
│                    Shadow 零 Prompt 交互流程                    │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   [1. 观察]          [2. 理解]          [3. 生成]              │
│   ┌─────────┐       ┌─────────┐       ┌─────────┐             │
...
以下展示 Shadow Web 端的真实交互场景，**完全没有对话、没有输入框，纯卡片式交互**：
...
⚠️ 设计原则：没有输入框，没有对话，所有交互通过点击卡片按钮完成
```
...
| 步骤 | 系统行为 | 用户行为 |
|------|---------|---------|
| 1 | 文件监控检测到下载文件夹变化 | 无需操作 |
| 2 | AI 分析文件，生成建议卡片 | 无需操作 |
| 3 | 展示上下文摘要 + 多个方案卡片 | 浏览卡片 |
| 4 | - | **点击**「预览」查看详情 |
| 5 | 展示预览界面，列出具体操作 | **点击**「确认执行」或「取消」|
| 6 | 执行操作，显示完成结果 | **点击**「完成」或「撤销」 |

**与传统 AI 助手对比**：

| 特性 | 传统 AI (ChatGPT/Claude) | Shadow A2UI |
|------|-------------------------|--------------|
| 启动方式 | 用户输入问题 | AI 主动感知并推送卡片 |
| 交互方式 | 文字对话 | 点击卡片按钮 |
| 输入框 | 必须有 | **完全没有** |
| 方案选择 | 用户打字描述需求 | 点击预定义卡片 |
| 执行确认 | "请帮我执行这个" | 点击「▶ 执行」按钮 |
| 界面元素 | 消息气泡 + 输入框 | 状态卡片 + 建议卡片 + 按钮 |
...
| **桌面常驻** | Tamagotchi 模式，始终陪伴 | ⭐⭐⭐ 符合 Shadow 定位 |
...
| 平台 | 定位 | AI 特性 | Shadow 差异 |
|------|------|---------|--------------|
| **Activepieces** | 低代码自动化 | AI Copilot 辅助构建 | Shadow 更主动，无需构建 |
| **n8n** | 工作流自动化 | AI 节点支持 | Shadow 更轻量，零配置 |
| **Zapier** | SaaS 集成 | AI Actions | Shadow 本地优先 |
| **Make** | 可视化自动化 | AI 场景推荐 | Shadow 自动发现场景 |

**核心差异**: Shadow 不需要用户预定义工作流，AI 自动发现并建议自动化机会。

#### 17.9.5 功能优先级建议

基于调研，建议 Shadow 按以下优先级增加功能：
...
### 18.2.1 2025 开源本地模型推荐

基于 [HuggingFace 开源 LLM 榜单](https://huggingface.co/blog/daya-shankar/open-source-llms)、[Artificial Analysis](https://artificialanalysis.ai/leaderboards/models) 等权威评测，推荐以下本地模型：

#### 文本模型（推理/代码）

| 模型 | 参数量 | 内存需求 | 特点 | Ollama 命令 |
|------|--------|---------|------|-------------|
| **Qwen3** ⭐ | 8B/14B/32B | 8-32GB | 代码生成和推理最强，超越 DeepSeek-R1 和 Llama 4 | `ollama pull qwen3:8b` |
| DeepSeek-R1 | 7B/14B/32B | 8-32GB | 数学推理强，复杂逻辑问题 | `ollama pull deepseek-r1:8b` |
| Llama 4 Scout | 17B | 16GB+ | Meta 最新，通用任务均衡 | `ollama pull llama4:scout` |
| Mistral | 7B | 8GB | 轻量高效，Apache 2.0 许可 | `ollama pull mistral` |

#### 视觉模型（屏幕理解）

| 模型 | 参数量 | 内存需求 | 特点 | Ollama 命令 |
|------|--------|---------|------|-------------|
| **Qwen2.5-VL** ⭐ | 7B/72B | 8-64GB | 7B 版本性能超越 Llama 3.2 Vision 11B | `ollama pull qwen2.5vl:7b` |
| Qwen3-VL | 30B/235B | 32GB+ | 最强多模态，256K 上下文 | `ollama pull qwen3-vl` |
| MiniCPM-V 2.6 | 8B | 6GB | 最小可用视觉模型 | `ollama pull minicpm-v` |
| Pixtral 12B | 12B | 12GB | Mistral 出品，指令跟随强 | `ollama pull pixtral` |

> 💡 **Shadow 默认配置**: 视觉模型使用 `qwen2.5vl:7b`，文本模型使用 `qwen3:8b`

**参考来源**:
- [Top 10 Open Source LLMs 2025](https://www.instaclustr.com/education/open-source-ai/top-10-open-source-llms-for-2025/)
...
| **用户痛点研究** | [user-pain-points-research.md](./user-pain-points-research.md) | 普通人使用 AI 的 10 大痛点及 Shadow 解决方案 |
...
*本文档由 Shadow Team 编写和维护。如有问题或建议，请提交 Issue。*
